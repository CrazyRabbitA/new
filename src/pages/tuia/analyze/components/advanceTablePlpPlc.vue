      ...
      <tbody v-if="showTable">
        <tr class="tr-items" v-for="(index, item) in backList" :key="index">
          <template v-for="(index, _item) in columns">
            ...
            <td :key="index" v-else>
               <span v-if ="_item.dataIndex === 'source'">
          ...
          <td>
            {{getPrecent(item.currentValue, 2)}}（{{item.quota11}}/{{item.quota21}}）
          </td>
          <td>
            {{getPrecent(item.contrastValue, 2)}}（{{item.quota12}}/{{item.quota22}}）
          </td>
          <td>
            {{getPrecent(item.afValue)}}
          </td>
          <td>

      ...
      <div class="advance-quota advance-quota-title">指标变化值</div>
        ...
        <template v-else>
          ...
          <p v-if="showTable('sdkUvSlot')" class="advance-tip advance-tip-slot">广告位维度：</p>
              <advance-table-sdk-uv v-if="showTable('sdkUvSlot')" :list.sync="totalList8" :columns.sync="columns3" :url="url"> </advance-table-sdk-uv>
        </template>
    </div>
  ...
  methods: {
    initData() {
      if (this.obj.quotaList) {
        ...
        this.totalList8 = this.obj.sdkUvSlot;
        ...
        this.totalList17 = this.obj.acPplpTradeAdvert; // 行业维度
          // 仅仅展示勾选项
        this.tabs = tabs.filter((item, index) => this.obj.quotaList.indexOf(item.eLabel) !== -1);

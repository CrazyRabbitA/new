<template>
  <pagetitle title="广告位维度实时数据" class="title"></pagetitle>
  <div class="page-content dimension cf">
    <div class="cf inp-filter">
      <div class="form-filter-group cf">
        <p class="filter-label fl  filter-label-soltid">广告位ID：</p>
        <input type="text" class="form-control fl" placeholder="不输入ID显示全局数据" v-model= "params.slotId">
      </div>
    ...
    <div class="broken-line-content">
      ...
      <div class="title">
        <h2>折线图</h2>
        <a :href="'/tuia/adslot/app/download?slotId=' + params.slotId + '&extraDate=' + params.extraDate" class="download fr"><i class="iconhandle mr5">&#xe606;</i>导出</a>
      </div>
  ...
  <top-modal v-if="isShowTopModalAsync" :show.sync="isShowTopModal" :top-type="'slotTop'":quota-type="quotaType" :select-day="params.extraDate" :close.sync="closeTopModal"></top-modal>
  <rank-modal v-if="isShowRankModalAsync" :show.sync="isShowRankModal" :quota-type="quotaType" :select-day="params.extraDate" :slot-id= "params.slotId" :close.sync="closeRankModal"></rank-modal>
...
<script>
  ...
  export default {
    ...
    route: {
      data(transition) {
        if (this.$route.params.id && this.$route.params.id !== 'undefined') {
          this.params.slotId = this.$route.params.id;
        }
        if (this.$route.params.data && this.$route.params.data !== 'undefined') {
          this.params.extraDate = this.$route.params.data;
        } else {
          if (getItem('extraDate2') !== void (0)) {
            this.params.extraDate = getItem('extraDate2');
          }
        }
    ...
    methods: {
      saveTime() {
        setItem('extraDate2', this.params.extraDate);
      },
      _getYesDay(dateStr) {
      ...
      */
      ...
      getDataList: function(transition) {
        ...
        this.$http.get('/tuia/adslot/app/line/chart', {
          params: this.params
        }).then((res) => {
          res = res.json();
          if (res.success) {

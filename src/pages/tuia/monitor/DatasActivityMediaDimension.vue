<template>
  <pagetitle title="活动媒体维度实时数据" class="title"></pagetitle>
  <div class="page-content dimension cf">
    <div class="cf inp-filter">
      <div class="form-filter-group cf">
        ...
        <input type="text" class="form-control fl form-control-act-app" placeholder="不输入ID显示全局" v-model= "params.activityId" @blur="onBlur()">
      </div>
      <div class="form-filter-group cf">
        <p class="filter-label fl  filter-label-mediaid">媒体ID：</p>
        <input type="text" class="form-control fl form-control-act-app" placeholder="不输入ID显示全局" v-model= "params.appId" @blur="onBlur()">
      </div>
    ...
    <div class="broken-line-content">
      ...
      <div class="title">
        <h2>折线图</h2>
        <a :href="'/tuia/activity/app/download?activityId=' + params.activityId + '&appId=' + params.appId + '&extraDate=' + params.extraDate + '&sourceType=' + params.sourceType" class="download fr"><i class="iconhandle mr5">&#xe606;</i>导出</a>
      </div>
  ...
  export default {
    ...
    route: {
      data(transition) {
        ...
        if (this.$route.params.app && this.$route.params.app !== 'undefined') {
          this.params.appId = this.$route.params.app;
        }
        if (this.$route.params.type && this.$route.params.type !== 'undefined') {
          this.params.sourceType = Number(this.$route.params.type);
        }
        if (getItem('extraDate4') !== void (0)) {
          this.params.extraDate = getItem('extraDate4');
        }
    ...
    methods: {
      saveTime() {
        setItem('extraDate4', this.params.extraDate);
      },
      _getYesDay(dateStr) {
      ...
      */
      ...
      getDataList: function(transition) {
        ...
        this.$http.get('/tuia/activity/app/line/chart', {
          params: this.params
        }).then((res) => {
          res = res.json();
          if (res.success) {

<template>
  <pagetitle title="活动维度实时数据" class="title"></pagetitle>
  <div class="page-content dimension cf">
    <div class="cf inp-filter">
      <div class="form-filter-group cf">
        <p class="filter-label fl  filter-label-id">活动ID：</p>
        <input type="text" class="form-control fl" placeholder="不输入ID显示全局数据" v-model= "params.activityId" @blur="onBlur()">
      </div>
    ...
    <div class="broken-line-content">
      ...
      <div class="title">
        <h2>折线图</h2>
        <a :href="'/tuia/activity/download?activityId=' + params.activityId + '&extraDate=' + params.extraDate + '&sourceType=' + params.sourceType" class="download fr"><i class="iconhandle mr5">&#xe606;</i>导出</a>
      </div>
  ...
  <top-modal v-if="isShowTopModalAsync" :show.sync="isShowTopModal" :top-type="'actTop'":quota-type="quotaType" :select-day="params.extraDate" :close.sync="closeTopModal"></top-modal>
  <rank-modal v-if="isShowRankModalAsync" :show.sync="isShowRankModal" :quota-type="quotaType" :select-day="params.extraDate" :act-id="params.activityId" :close.sync="closeRankModal" :source-type="params.sourceType"></rank-modal>
...
<script>
  ...
  export default {
    ...
    route: {
      data(transition) {
        ...
        if (this.$route.params.data && this.$route.params.data !== 'undefined') {
          ...
          if (getItem('extraDate3') !== void (0)) {
            this.params.extraDate = getItem('extraDate3');
          }
        }
    ...
    methods: {
      delTime() {
        this.params.extraDate = '';
      },
      saveTime() {
        setItem('extraDate3', this.params.extraDate);
      },
      _getYesDay(dateStr) {
      ...
      */
      ...
      getDataList: function(transition) {
        ...
        this.$http.get('/tuia/activity/line/chart', {
          params: this.params
        }).then((res) => {
          res = res.json();
          if (res.success) {

          ...
          <tr class="tr-items" v-for="(index, item) in rankTopData.currentTable" track-by="$index">
            <template v-for="column in columns">
              <template v-if="column.render">
                <td v-if="(currentQuota === 23 || currentQuota === 17 || currentQuota === 16 || currentQuota === 20 || currentQuota === 21 || currentQuota === 24) && (column.dataIndex === 'currentValue' || column.dataIndex === 'contrastValue'|| column.dataIndex === 'variation')">{{getPrecent(item[column.dataIndex])}}</td>
                <td v-else>

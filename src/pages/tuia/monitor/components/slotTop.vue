        ...
        <tbody v-if="data && data.length !== 0">
          <tr class="tr-items" v-for="(index, item) in data" track-by="$index">
            <template v-for="column in columns">
              ...
              <template v-else>
                <td v-if="column.dataIndex === 'slotId'"><a v-link="{ path: '/monitor/adslotDimension/' + item.slotId + '/' + selectDay }" target="_blank">{{item[column.dataIndex]}}</a></td>
...
export default {
  ...
  methods: {
    getDataList() {
      this.loadshow = true;
      this.$http.get(`/tuia/adslot/app/top/list?orderType=${this.params.orderType}&orderBy=${this.params.orderBy}`, {}).then(
        res => {
          res = res.json();
          if (res.success) {

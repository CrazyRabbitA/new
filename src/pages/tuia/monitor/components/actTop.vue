        ...
        <tbody v-if="data && data.length !== 0">
          <tr class="tr-items" v-for="(index, item) in data" track-by="$index">
            <template v-for="column in columns">
              ...
              <template v-else>
                <td v-if="column.dataIndex === 'activityId'"><a v-link="{ path: '/monitor/activitydimension/' + item.activityId + '/' + selectDay }" target="_blank">{{item[column.dataIndex]}}</a></td>
...
export default {
  ...
  methods: {
    getDataList() {
      this.loadshow = true;
      this.$http.get(`/tuia/activity/app/top/list?orderType=${this.params.orderType}&orderBy=${this.params.orderBy}`, {}).then(
        res => {
          res = res.json();
          if (res.success) {

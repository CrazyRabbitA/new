<template>
  <div class="modal-body">
    ...
    <div class="rule-list uprank-rule-list">
            ...
            <template v-for="column in columns">
              ...
              <template v-else>
                  <td v-if="column.dataIndex === 'activityId'">
                    ...
                    <a v-else v-link="{ path: '/monitor/activitydimension/' + item.activityId }" target="_blank">{{item[column.dataIndex]}}</a>
                  </td>
      ...
      <tableempty v-if="!loadshow && (data && data.length === 0)"></tableempty>

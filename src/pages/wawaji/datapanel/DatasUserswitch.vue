  ...
  <div>
    ...
    <div class="chart-wrap">
      <div class="chart-item item-main">
        ...
        <div id="newuserMain" class="newuser-income-chart" style="height:480px;width:100%"></div>
      </div>
      <div class="chart-item item-other">
        ...
        <div v-show="newuserCanvas2">
          ...
          <div id="newuserMain2" class="newuser-income-chart" style="height:480px;width:100%"></div>
        </div>
      </div>
    </div>
...
export default {
  ...
  methods: {
    ...
    getHopperList(time, dom) {
      ...
      this.$http
        .get(`/wawaji/cranegame/path/userpath/hopper?${p}`).then((res) => {
          res = res.json();
          if (res.success) {
            this.newuserChart(res.data.hopper, dom, 'userswitch');
          }
        },
        res => {
    ...
    getTableList() {
      ...
      this.$http
        .get(`/wawaji/cranegame/path/userpath/table?${p}`).then((res) => {
          res = res.json();
          if (res.success) {
    ...
    getTablewithpage(orderBy, orderType) {
      ...
      this.$http
        .post('/wawaji/cranegame/path/userpath/tablewithpage?', p).then((res) => {
          res = res.json();
          if (res.success) {
            let arr = res.data.table;
            arr.unshift(this.collect);
    ...
    outTotalData() {
      ...
      const goLink = `/wawaji/cranegame/path/userpath/table/download?${p}`;
      window.open(goLink);
    },
    onPageChange(val) {
      // 当前页不发送请求

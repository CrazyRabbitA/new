<template>
  ...
  <div class="chart-area">
    ...
    <div id="goodsTimeChart" class="echart" v-el:echarts
...
<script>
  ...
  import goodsTimeMixins from './mixins/goodsTimeMixins.js';
  import goodsTimeChartMixins from './mixins/goodsTimeChartMixins.js';
  const quotaArr = [
    {
      label: '发券量',
      id: 'launchCount'
  ...
  export default {
    mixins: [dcMixins, goodsTimeMixins, goodsTimeChartMixins],
    data() {
      return {
        data: {},
        columns: columns,
    ...
    route: {
      data: function() {
        if (this.$route.params.id) {
          ...
          this.getDataList(this.data, this.curQuota);
        }
      }
    },
    methods: {
      ...
      getDataList() {
        this.$http.get('/qiho/item/dhour', { params: this.params }).then((res) => {
          res = res.json();
          if (res.success) {

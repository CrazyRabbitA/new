<template>
  <div class="cx">
      <div class="form-inline bottomline tuiafilter filter-style">
         ...
         <div class="form-filter-group">
            <d-button size="large" :disabled="errorShow" @click="filterCoupons" class="margin8 d-button--qiho wide-button">筛选</d-button>
        </div>
      </div>
  </div>
  <div class="listTable">
  ...
  <loading :show="tuiaCouponLoading"></loading>
</template>
...
<script>
  ...
  import coupontablemixins from './mixins/couponTableMixins';

  export default {
    mixins: [dcMixins, coupontablemixins],
    data() {
      return {
        data: [],
        tuiaCouponLoading: false,
        params: {
          pageNum: 1,
          pageSize: 20,
          itemId: '',
    ...
    route: {
      data: function(transition) {
        if (this.$route.params.id) {
          ...
          this.getCouponList(this.params, transition);
        }
      }
    },
    methods: {
       ...
       * 获取商品列表
       */
      getCouponList: function(options, transition) {
        this.tuiaCouponLoading = true;
        this.$http.post('/logistics/queryAdvertNumStatisticsDayPage', options).then((res) => {
          res = res.json();
          if (res.success) {

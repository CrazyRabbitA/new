    ...
    <table class="recommend-list mt10 summary">
      ...
      <tbody>
        <tr class="tr-items-summary">
          <td>
            <!-- 百分数 -->
            <template v-if="currentQuota === 201 || currentQuota === 202 || currentQuota === 204">
    ...
    <div class="rule-list uprank-rule-list">
      <table class="recommend-list mt10">
        ...
        <tbody v-if="data && data.table && data.table.length !== 0">
          <tr class="tr-items" :key="index" v-for="(index, item) in rankTopData.currentTable" track-by="$index">
            <template v-for="(index,column) in columns">
              <template v-if="column.render">
                <td v-if = "currentQuota === 201 || currentQuota === 202 || currentQuota === 204">
                   {{(item[column.dataIndex] === 'Infinity' || item[column.dataIndex] === Infinity) ? '∞':
                    getPrecent(item[column.dataIndex])}}
                </td>
                <template v-else >
                  <td v-if = "currentQuota === 101 || currentQuota === 102 ||currentQuota === 103 || currentQuota === 104 || currentQuota === 105">
              ...
              <template v-else>
                <td v-if="column.dataIndex === 'appId'">
